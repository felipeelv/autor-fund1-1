...
origem: unidades-07-08-p01-simbolo-mais-e-igual-rascunho.md
---
...
azul `#2F6FD0` na abertura (conceito, é a primeira página da unidade), verde
`#43A66B` no núcleo dos dois símbolos (verificação — "veja como funciona"),
amarelo `#F6C945` no exemplo de conta (destaque/"para lembrar"). Texto e
contornos em grafite `#263238`. Colagem com papel liso, bordas rasgadas, fita
e sombra curta; sem grade digital, sem 3D decorativo.
...
- núcleo 3, papel amarelo, tabela `COM OBJETOS` / `A CONTA` com as quatro
  ...
  amarelos), cada linha desenhando exatamente os emojis/objetos indicados
  antes do sinal de igual e o total depois; abaixo, o post-it
...
Símbolo `+` e símbolo `=` nunca aparecem sozinhos — sempre ao lado dos
objetos ou do número que representam. Contagem exata em cada linha da
tabela de exemplos: 2+3=5, 3+2=5, 4+1=5, 1+4=5 — todos os grupos somam
exatamente 5 objetos, sem objeto a mais ou a menos. Não fundir os três
