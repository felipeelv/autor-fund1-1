...
> Trechos: Conhecendo o Símbolo - (Menos) (linhas 162-177), Exemplos de Contas de Separar (linhas 193-207)
...
- núcleo 1, papel azul, ocupando a maior parte da página: o símbolo `-`
  grande e colorido, com 5 blocos de um grupo e uma seta mostrando 2 deles
  saindo (riscados com X), ilustrando "tirar"; a frase-explicação ao lado
  (`O símbolo - se chama MENOS...` / `Quando vemos o -, sabemos que vamos
  retirar uma quantidade!`); abaixo, o post-it `📌 VEJA COMO FUNCIONA` com a
  frase-exemplo;
- núcleo 2, papel amarelo, tabela `COM OBJETOS` / `A CONTA` com as quatro
  linhas de exemplos de separar (maçãs, estrelas, círculos azuis, círculos
  amarelos), cada linha desenhando exatamente os objetos indicados, os
  objetos retirados marcados com `❌`, e o total restante depois do `=`;
  abaixo, o post-it `💡 PARA LEMBRAR` com a frase final.
...
Símbolo `-` nunca aparece sozinho — sempre ao lado dos objetos sendo
retirados. Contagem exata em cada linha da tabela de exemplos: 5-2=3,
4-1=3, 6-3=3, 7-4=3 — o número de `❌` corresponde exatamente ao segundo
número da conta. Não fundir os dois núcleos. Sem fotografia, sem balão de
fala, sem mascote.
