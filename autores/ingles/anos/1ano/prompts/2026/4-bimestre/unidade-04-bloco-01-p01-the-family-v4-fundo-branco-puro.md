Use case: scientific-educational
Asset type: página 1 de uma sequência didática de Inglês do 1º ano

## PEDIDO

Criar uma única página vertical 2:3, pronta para impressão, que abra a
unidade com o que a criança vai aprender e apresente os seis membros da
família em inglês (mom, dad, brother, sister, grandma, grandpa) por meio de
ilustração concreta, tradução e a estrutura "This is my...". Fecha com uma
curiosidade sobre apelidos carinhosos dos pais e avós. Fica de fora desta
página: a música "This is my Family" e os pronomes he/she, que pertencem às
páginas seguintes da unidade.

Título previsto: The Family

## SISTEMA VISUAL

Seguir integralmente `autores/ingles/direcao/PADRAO-VISUAL-1ANO.md`: papel
branco puro (fundo `#FFFFFF`, sem tom creme ou amarelado), colagem de papel
recortado com pintura guache, sketchnote profissional (setas, círculos,
conectores) e visual note-taking orgânico, sem grade rígida. Ilustração
semirrealista de contornos orgânicos, títulos grandes em sans-serif
arredondada, seis cores didáticas vivas sobre estrutura em grafite. Uma
criança brasileira pode aparecer apontando para um membro da família, sem
ocupar mais de um quinto da página.

## COMPOSIÇÃO E TÍTULO

- fundo da página inteira branco puro, sem faixa, painel ou textura colorida
  cobrindo grandes áreas — cor entra só nos recortes de colagem e acentos;
- topo compacto com o título "THE FAMILY";
- logo abaixo do título, uma faixa curta "NESTA UNIDADE VOCÊ VAI APRENDER"
  com os quatro itens da lista (vocabulário, possessivo, estruturas, músicas
  e gestos), em corpo pequeno — é a abertura da unidade, não um núcleo
  ilustrado;
- núcleo central "MEET THE FAMILY" com os seis membros, cada um como um
  recorte de colagem com a ilustração da pessoa, a palavra em inglês em
  destaque e a tradução logo abaixo: MOM (mamãe), DAD (papai), BROTHER
  (irmão), SISTER (irmã), GRANDMA (vovó), GRANDPA (vovô);
- cor fixa de cada pessoa, sem repetir cor entre membros diferentes: MOM
  vermelho, DAD azul, BROTHER verde, SISTER roxo, GRANDMA laranja, GRANDPA
  amarelo — a mesma paleta das páginas 2, 3 e 4 desta unidade;
- faixa "DID YOU KNOW?" com a curiosidade sobre apelidos carinhosos (mommy,
  daddy, granny/nana, grandad/papa), em corpo menor, como nota lateral;
- bloco "THIS IS MY..." com a estrutura e as seis frases-exemplo, uma por
  membro da família, cada frase próxima ao recorte correspondente do núcleo
  central (seta ou linha curta ligando frase e ilustração);
- caminho de leitura: título → o que você vai aprender → seis membros da
  família → curiosidade → estrutura "this is my...".

## TEXTOS EXATOS

Renderizar cada um exatamente uma vez, na ordem abaixo. Preservar acentos,
maiúsculas, pontuação e negrito conforme indicado.

- THE FAMILY
- NESTA UNIDADE VOCÊ VAI APRENDER
- Membros da família: mom, dad, brother, sister, grandma, grandpa
- Possessivo: my
- Estruturas: This is my... / I love my...
- Músicas e gestos para aprender brincando
- MOM — mamãe
- DAD — papai
- BROTHER — irmão
- SISTER — irmã
- GRANDMA — vovó
- GRANDPA — vovô
- DID YOU KNOW?
- Em inglês, existem várias formas carinhosas de chamar os pais: MOM também
  pode ser MOMMY (mamãe), DAD também pode ser DADDY (papai), GRANDMA também
  pode ser GRANNY ou NANA, e GRANDPA também pode ser GRANDAD ou PAPA. Cada
  família escolhe como prefere chamar!
- THIS IS MY...
- This is my mom. (Esta é minha mãe)
- This is my dad. (Este é meu pai)
- This is my brother. (Este é meu irmão)
- This is my sister. (Esta é minha irmã)
- This is my grandma. (Esta é minha avó)
- This is my grandpa. (Este é meu avô)

### Nota de correção

Correção sobre a v3 (17/09/2026): o fundo estava especificado como "papel
branco quente", em desacordo com `CLAUDE.md` seção 1 ("fundo branco puro").
Nicolas Basso apontou o problema ao ver a imagem gerada. Corrigido para
branco puro `#FFFFFF` aqui e em `PADRAO-VISUAL-1ANO.md`. Nenhum outro texto
muda em relação à v3.

Mantêm-se as notas de correção anteriores: o cabeçalho de abertura fica em
português (corrigido na v3); o extrator de literais não capturou o
vocabulário da seção VOCABULÁRIO da fonte, copiado manualmente e conferido
contra a fonte bruta (v1).

## TRAVAS

Valem as travas de `autores/ingles/anos/1ano/REGRAS.md` e o `prompt_sufixo` do autor, que o gerador
injeta automaticamente.

Invioláveis, independentes de ano: não inventar número, nome próprio, data,
lugar, povo ou termo ausente da fonte; renderizar cada texto literal exatamente
uma vez; nenhum texto além da lista de TEXTOS EXATOS. Não usar mascote,
logotipo, número de página, fotografia de banco de imagens ou estética de
desenho animado comercial. Não trocar a correspondência entre palavra em
inglês e ilustração/tradução.
