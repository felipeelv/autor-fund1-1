...
""",
        ...
        self.assertEqual(task.options.model, "qwen/qwen-image-3-pro")
        self.assertEqual(task.options.aspect_ratio, "2:3")
        self.assertEqual(task.options.resolution, "2k")
        self.assertEqual(task.options.output_format, "png")

    def test_openrouter_project_rejects_pixel_size(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            base = Path(directory)
            root = base / "project"
            root.mkdir()
...
""",
            ...
            with self.assertRaisesRegex(ConfigError, "tamanho=auto"):
                load_project(root, project, storage)
